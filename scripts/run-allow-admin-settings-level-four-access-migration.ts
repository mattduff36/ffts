...
const sqlFile = 'supabase/migrations/20260528_allow_admin_settings_level_four_access.sql';
...
async function runMigration(conn: string) {
  ...
  try {
    console.log('Running allow admin settings Level 4 access migration...');
    ...
    if (Number(result?.settings_min || 0) !== 4 || result?.settings_full_only !== false) {
      throw new Error('Admin Settings Level 4 access verification failed.');
    }

    console.log('Allow admin settings Level 4 access migration complete.');
  } finally {
    await client.end();
  }
}
