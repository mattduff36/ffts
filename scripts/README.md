...
Cursor commands under `.cursor/commands/` cover `/workflow-review`, `/finalise`, `/finalise-full`, `/fap`, `/ffap`, `/createinvoice`, `/cleancodebase`, and `/fixerrors`. `/finalise` and `/finalise-full` do not authorize a push. Short aliases `/fap` and `/ffap` also do **not** authorize a push; only explicit phrases (`finalise and push`, `finalise full and push`, `finalise:push`, or `push to GitHub`) authorize `npm run finalise:push` / `npm run finalise:full:push`. `/fixerrors` is a trusted operational command under safety contract `fixerrors-exact-snapshot-v1`: default `npm run fixerrors` exports a repeatable-read snapshot only; destructive cleanup requires the exact printed bound `--cleanup` command after confirmation.
