...
describe('TEE V2.2 FFTS project context', () => {
  it('TEE-DOCS-001 / TEE-PUSH-001: fap and ffap authorize their push variants', () => {
    ...
    expect(fap).toMatch(/git push origin <full-sha>:refs\/heads\/main/);
    expect(ffap).toMatch(/git push origin <full-sha>:refs\/heads\/main/);
    ...
    expect(finaliseCommands).toMatch(/ordinary work runs `npm run finalise`/i);
    expect(finaliseCommands).toMatch(/ordinary work runs `npm run finalise:full`/i);
